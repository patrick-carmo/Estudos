...
const filtrarDados = (profissao?: string): Usuario[] => {
  const bd = lerArquivo() as Usuario[]

  if (profissao) {
    const usuarios = bd.filter(dados => dados.profissao?.toLowerCase() === profissao.toLowerCase())
    return usuarios
  }
...
console.log(filtrarDados('desenvolvedor back-end'))
